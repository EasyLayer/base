...
describe('TransactionsBatchQueue', () => {
  ...
  it('should not enqueue batch with incorrect height', () => {
    const batch = new TestTransactionsBatch(1, 'hash1', null, 0);
    const result = queue.enqueue(batch);
  ...
  it('should dequeue batch', () => {
    const batch1 = new TestTransactionsBatch(0, 'hash1', null, 0);
    const batch2 = new TestTransactionsBatch(0, 'hash1', null, 1, true);
    const batch3 = new TestTransactionsBatch(1, 'hash2', 'hash1', 0, true);
  ...
  it('should peek first batch', async () => {
    ...
    expect(firstBatch?.blockHeight).toBe(0);
  });
  ...
  it('should fetch a batch by index from inStack using binary search', () => {
    ...
    queue.enqueue(batch2);
    queue.enqueue(batch3);
    const result = queue.fetchBatchFromInStack(1, 'hash2', 0);
  ...
  it('should return undefined if batch is not found in inStack using binary search', () => {
    const batch = new TestTransactionsBatch(0, 'hash1', null, 0);
    queue.enqueue(batch);
    const result = queue.fetchBatchFromInStack(0, 'hash1', 1);
  ...
  it('should fetch a batch by index from outStack using binary search', () => {
    ...
    const result = queue.fetchBatchFromOutStack(0, 'hash1', 1);
  ...
  it('should fetch batches correctly after transferItems', () => {
    ...
    queue.enqueue(batch3);
    queue['transferItems'](); // Manually trigger transferItems
    const resultInStack = queue.fetchBatchFromInStack(0, 'hash1', 0);
    const resultOutStack = queue.fetchBatchFromOutStack(0, 'hash1', 0);
  ...
  it('should not enqueue batch if max block height is reached', () => {
    queue.maxBlockHeight = 0; // Set max block height for testing
    const batch1 = new TestTransactionsBatch(0, 'hash1', null, 0);
    const batch2 = new TestTransactionsBatch(0, 'hash1', null, 1, true);
    const batch3 = new TestTransactionsBatch(1, 'hash2', 'hash1', 0, true);
    queue.enqueue(batch1);
