...
export class ReadStateExceptionHandlerService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    this.subscription = this.unhandledExceptionBus.subscribe((exception: UnhandledExceptionEvent) => {
        this.log.error('Read State Unhandled Exception:', { exception }, this.constructor.name);
