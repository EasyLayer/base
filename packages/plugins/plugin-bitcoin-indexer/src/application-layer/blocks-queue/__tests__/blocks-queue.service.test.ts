...
describe('BlocksQueueService', () => {
  ...
  describe('blocksIterator', () => {
    it('should yield blocks when they are available in the queue', async () => {
      ...
      jest.spyOn(service['blockQueue'], 'peekFirstBlock').mockResolvedValueOnce(blockMock1).mockResolvedValueOnce(blockMock2).mockResolvedValue(undefined);
      jest.spyOn(service as any, 'peekFirstBlock').mockResolvedValueOnce(blockMock1).mockResolvedValueOnce(blockMock2).mockResolvedValue(undefined);
  ...
  describe('startQueueIteratting', () => {
    it('should process blocks and call indexBlock', async () => {
      const blockMock: Block = { height: BigInt(1), hash: 'hash 1', tx: [] };
      jest.spyOn(service as any, 'blocksIterator').mockReturnValue((async function* () {
        yield blockMock;
      })());
    ...
    it('should wait for block to be confirmed before processing the next block', async () => {
      ...
      const blocksIteratorSpy = jest.spyOn(service as any, 'blocksIterator').mockReturnValue((async function* () {
        yield blockMock1;
        yield blockMock2;
      })());
  
      const indexBlockSpy = jest.spyOn(service['blocksCommandFactory'], 'indexBlock').mockResolvedValue(undefined);
      const dequeueSpy = jest.spyOn(service['blockQueue'], 'dequeue').mockImplementation(() => blockMock1);
  
      // Устанавливаем начальный промис и резолвер
      service['blockProcessedPromise'] = new Promise<void>(resolve => {
        service['resolveNextBlock'] = resolve;
      });
  
      // Запускаем метод startQueueIteratting
      const startIteratingPromise = service['startQueueIteratting']();
  
      // Подтверждаем первый блок
      await service.confirmIndexBlock();
  
      // Меняем возвращаемое значение dequeue для следующего блока
      dequeueSpy.mockImplementation(() => blockMock2);
  
      // Подтверждаем второй блок
      await service.confirmIndexBlock();
      ...
      expect(service['blockQueue'].dequeue).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('loading', () => {
    it('should stop loading when the queue is full', async () => {
      ...
      // Изначально заполняем очередь до максимального размера
      queueLength = mockSystemConfig.BITCOIN_INDEXER_BLOCKS_QUEUE_MAX_SIZE;

      // Запускаем метод loading
      await service['loading']();

      // Убедимся, что enqueue больше не вызывается после того, как очередь достигла максимального размера
    ...
    it('should load blocks and enqueue them when the queue is not full', async () => {
      ...
      // Убедимся, что enqueue был вызван и длина очереди увеличилась
    ...
    it('should continue loading blocks until the queue is full', async () => {
      ...
      // Убедимся, что enqueue вызывается нужное количество раз
      expect(service['blockQueue'].enqueue).toHaveBeenCalledTimes(mockSystemConfig.BITCOIN_INDEXER_BLOCKS_QUEUE_MAX_SIZE + 1);
    });
    ...
    it('should call loadBlockWithRetry the correct number of times', async () => {
      ...
      // Убедимся, что loadBlockWithRetry вызывается нужное количество раз
  ...
  describe('loadBlockWithRetry', () => {
    ...
    it('should retry loading block on failure', async () => {
      const blockMock: Block = { height: BigInt(1), hash: 'hash 1', tx: [] };
      jest.spyOn(service as any, 'loadBlock').mockRejectedValueOnce(new Error('mock error')).mockResolvedValue(blockMock);
    ...
    it('should throw an error after max retries', async () => {
      ...
      await expect(service['loadBlockWithRetry'](BigInt(1))).rejects.toThrow('Failed to load block at height 1 after 3 attempts: Error: mock error');
  ...
  describe('loadBlock', () => {
    it('should load block using worker pool', async () => {
      ...
      expect(result).toBe(blockMock);
      expect(service['workerPool'].run).toHaveBeenCalledWith({ height: BigInt(1), providersConnectionOptions: undefined });
    });
  });
  ...
  describe('confirmIndexBlock', () => {
    it('should dequeue a block and resolve the next block promise', async () => {
      const blockMock: Block = { height: BigInt(1), hash: 'hash 1', tx: [] };
      jest.spyOn(service['blockQueue'], 'dequeue').mockReturnValue(blockMock);
      jest.spyOn(service as any, 'resolveNextBlock');
  
      await service.confirmIndexBlock();
  
      expect(service['blockQueue'].dequeue).toHaveBeenCalled();
      expect(service['resolveNextBlock']).toHaveBeenCalled();
    });
  
    it('should throw an error if the block cannot be confirmed', async () => {
      jest.spyOn(service['blockQueue'], 'dequeue').mockReturnValue(undefined);
  
      await expect(service.confirmIndexBlock()).rejects.toThrow('The block cannot be confirmed');
      expect(service['blockQueue'].dequeue).toHaveBeenCalled();
    });
  });
});
