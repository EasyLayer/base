...
(async () => {
  ...
  const app = await NestFactory.createApplicationContext(BitcoinBalancesIndexerModule, {
    logger: false,
  });

  const outputsReadService = app.get(OutputsReadService);
  ...
  program
    ...
    .action(async (options) => {
      ...
      await outputsReadService.createIndexes(options.table, options.columns);
